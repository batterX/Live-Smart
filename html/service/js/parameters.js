		...
		if(response) {
			switch(response.toLowerCase()) {
				case 'batterx h3':
					...
					$('#enable_feedInLimit_lifepo_Wp, #enable_feedInLimit_carbon_Wp').val(3000);
					break;
				case 'batterx h5':
					deviceModel = "batterx h5";
					$('._h5').css('display', 'flex');
					$('#enable_feedInLimit_lifepo_Wp, #enable_feedInLimit_carbon_Wp').val(5000);
					break;
				case 'batterx h5-eco':
					deviceModel = "batterx h5-eco";
					$('._h5e').css('display', 'flex');
					$('#enable_feedInLimit_lifepo_Wp, #enable_feedInLimit_carbon_Wp').val(5500);
					break;
				case 'batterx h10':
					deviceModel = "batterx h10";
					$('._h10').css('display', 'flex');
					$('#enable_feedInLimit_lifepo_Wp, #enable_feedInLimit_carbon_Wp').val(10000);
					break;
				default:
...
$('#applyLiFePO').on('click', function() {
	...
	var enable_feedInLimit = $('#enable_feedInLimit_lifepo').is(':checked');
	...
	var temp_maxGridFeedInPower = $('#enable_feedInLimit_lifepo_Wp').val();
	if(enable_feedInLimit) {
		var limitationPercentage = $('#enable_feedInLimit_lifepo_percent').val();
		if(temp_maxGridFeedInPower != "") {
			temp_maxGridFeedInPower = Math.max(Math.round(parseInt(temp_maxGridFeedInPower) * parseInt(limitationPercentage) / 100), 50);
		} else {
				 if(deviceModel == 'batterx h3')     temp_maxGridFeedInPower = Math.max(math.round( 3000 * parseInt(limitationPercentage) / 100), 50);
			else if(deviceModel == 'batterx h5-eco') temp_maxGridFeedInPower = Math.max(math.round( 5500 * parseInt(limitationPercentage) / 100), 50);
			else if(deviceModel == 'batterx h5')     temp_maxGridFeedInPower = Math.max(math.round( 5000 * parseInt(limitationPercentage) / 100), 50);
			else if(deviceModel == 'batterx h10')    temp_maxGridFeedInPower = Math.max(math.round(10000 * parseInt(limitationPercentage) / 100), 50);
		}
	} else {
...
$('#applyCarbon').on('click', function() {
	...
	var enable_feedInLimit = $('#enable_feedInLimit_carbon').is(':checked');
	...
	var temp_maxGridFeedInPower = $('#enable_feedInLimit_carbon_Wp').val();
	if(enable_feedInLimit) {
		var limitationPercentage = $('#enable_feedInLimit_carbon_percent').val();
		if(temp_maxGridFeedInPower != "") {
			temp_maxGridFeedInPower = Math.max(Math.round(parseInt(temp_maxGridFeedInPower) * parseInt(limitationPercentage) / 100), 50);
		} else {
				 if(deviceModel == 'batterx h3')     temp_maxGridFeedInPower = Math.max(math.round( 3000 * parseInt(limitationPercentage) / 100), 50);
...
$('#numberOfModules, #enable_feedInLimit_lifepo, #enable_feedInLimit_lifepo_percent, #enable_feedInLimit_lifepo_Wp').on('change input', function() {
	
	var enable_feedInLimit = $('#enable_feedInLimit_lifepo').is(':checked');
	if(enable_feedInLimit) $('#enable_feedInLimit_lifepo_Wp').removeAttr('disabled');
	else $('#enable_feedInLimit_lifepo_Wp').attr('disabled', true);
	...
	$('.lifepo .cutoffVoltage'                ).text('45.0 V');
...
$('#batteryCapacity, #numberOfBatteries, #depthOfDischarge, #enable_feedInLimit_carbon, #enable_feedInLimit_carbon_percent, #enable_feedInLimit_carbon_Wp').on('change input', function() {
	
	var enable_feedInLimit = $('#enable_feedInLimit_carbon').is(':checked');
	if(enable_feedInLimit) $('#enable_feedInLimit_carbon_Wp').removeAttr('disabled');
	else $('#enable_feedInLimit_carbon_Wp').attr('disabled', true);
